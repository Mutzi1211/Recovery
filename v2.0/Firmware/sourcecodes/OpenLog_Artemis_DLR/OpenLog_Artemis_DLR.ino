...
const int FIRMWARE_VERSION_MAJOR = 1;  // war 2
const int FIRMWARE_VERSION_MINOR = 0;  // war 5
...
//DLR add Servo Interface
//-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
#include <Servo.h>  // Click here to get the library: http://librarymanager/All#Servo
...
#define DUMP(varname) \
  { \
    Serial.printf("%s: %d\r\n", #varname, varname); \
    if (settings.useTxRxPinsForTerminal == true) Serial1.printf("%s: %d\r\n", #varname, varname); \
  }
#define SerialPrintf1(var) \
  { \
    Serial.printf(var); \
    if (settings.useTxRxPinsForTerminal == true) Serial1.printf(var); \
  }
#define SerialPrintf2(var1, var2) \
  { \
    Serial.printf(var1, var2); \
    if (settings.useTxRxPinsForTerminal == true) Serial1.printf(var1, var2); \
  }
#define SerialPrintf3(var1, var2, var3) \
  { \
    Serial.printf(var1, var2, var3); \
    if (settings.useTxRxPinsForTerminal == true) Serial1.printf(var1, var2, var3); \
  }
#define SerialPrintf4(var1, var2, var3, var4) \
  { \
    Serial.printf(var1, var2, var3, var4); \
    if (settings.useTxRxPinsForTerminal == true) Serial1.printf(var1, var2, var3, var4); \
  }
#define SerialPrintf5(var1, var2, var3, var4, var5) \
  { \
    Serial.printf(var1, var2, var3, var4, var5); \
    if (settings.useTxRxPinsForTerminal == true) Serial1.printf(var1, var2, var3, var4, var5); \
  }
...
void setup() {
  ...
  // DLR reset the parachute at setup
  Serial.println("Servo setup");
  servo.attach(11);          //DLR: connect servo to Pin11
  parachute_open = reset();  //DLR: reset parachute at setup
}
...
void loop() {
  ...
  if (now - old >= 100) {
    old = now;  // Zeitpunkt aktualisieren
    // dlr
    if (abs(myICM.accZ()) < 100) {
      zerog++;
