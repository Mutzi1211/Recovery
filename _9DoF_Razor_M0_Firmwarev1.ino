/******************************************************************************
Original : SparkFun 9DoF Razor M0 Example Firmware Jim Lindblom @ SparkFun Electronics
Original creation date: November 22, 2016 https://github.com/sparkfun/9DOF_Razor_IMU/Firmware

Editet by Sebastian Plamauer ...2017
github ... turbinenreiter...
Editet by DLR-SL_OP Projekt: MoRaBa-Recovery-Team
Matthias Schuhbauer 21.08.2018
github ... "gitrepo"...

Firmware for the 9DoF-Razor-IMU in the DLR-SL-OP MoRaBa-Recovery Experiment.
Its implementet in an Module for a compressed-air-water-rocket used in student educational Experiments.
Purpose is to Ejekt an paracute shortly after the turning point in the flight Path 
and to save an Log file with acceleration an preasure data to the onboard SD-card.
Changes from Orginal are mostly marked with "//DLR:"
...
BME280 env;  //DLR: instance of the BME280
Servo servo; //DLR: instance of Servo
...
void setup()
{
  ...
  servo.attach(12);  //DLR: connect servo to Pin12
  parachute_open = reset(); //DLR: reset paracute at setup
...
//DLR: initialising BME280 /env 
  env.settings.commInterface = I2C_MODE;  // DLR: Communikation is I2C
  env.settings.I2CAddress = 0x77;         // DLR: I2C Adress of BME280 0x77
...
int pos = 0;
/*DLR: bool reset(): 
 * reset Servo Position to 90deg -> closes/locks door 
 * slowly increasing Servo Position by 1 deg at each repetition till it reaches 90deg 
 * returns false as Status
*/
bool reset() {
  for (; pos < 90; pos++) { 
    servo.write(pos);
...
/*DLR: bool deploy(): 
 * sets Servo Position to 0 deg -> opens/unlocks door 
 * slowly decreasing Servo Position by 1 deg at each repetition till it reaches 0deg 
 * returns false as Status
*/
bool deploy() {
  for (; pos > 0; pos--) {
...
void logIMUData(void)
{
  ...
  {
    /* DLR: write Preasure, Hight, Humidity, Temperatur and Paracute-Open-Flag(1->Open)
    * data from the BME280 to the SD-Card
    * Logging: ...P[hpa], h[m.ü.NN], hum[%], T[C], POF[0/1]
    */
